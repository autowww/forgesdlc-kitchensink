...
async function loadAiFindingsByRule(auditDataPath) {
  ...
  const map = new Map();
  const pool = [...(raw.findings || [])];
  const aiPath = path.join(path.dirname(auditDataPath), 'ai-audit', 'ai-audit-data.json');
  try {
    const ai = JSON.parse(await fs.readFile(aiPath, 'utf8'));
    pool.push(...(ai.scoreableFindings || ai.findings || []));
  } catch {
    /* no batch ai file */
  }
  for (const f of pool) {
