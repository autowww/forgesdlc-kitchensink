...
import {
  findingsFromAppPrimitiveMarkersReport,
  isValidKsHashToken,
  run,
} from '../design-rules/deterministic/generated/det-app-primitive-markers.check.js';
...
test('findingsFromAppPrimitiveMarkersReport flags missing and invalid KS attrs', () => {
  const findings = findingsFromAppPrimitiveMarkersReport({
...
test('run uses metrics.appPrimitiveMarkersReport when provided', async () => {
  const findings = await run({
    metrics: {
      url: 'https://example.test/showcase/forge-react-primitives.html',
      appPrimitiveMarkersReport: {
