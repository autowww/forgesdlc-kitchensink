...
- Files indexed: 1633
- URLs analyzed this run: **2**
- **Crawl breadth:** **full** within `--max-pages` (`--breadth-crawl` / `--stop-disable`). **Priority batch** shows up to ten worst for readability (**42** in **All findings this run** and per-page arrays in `audit-data.json`).
- **Audit run id:** `8f895c64766e014e` — **schema:** `2` (see `audit-data.json`)
- **Generated at (UTC):** `2026-05-25T13:42:33.120Z`
...
Live crawl **completed** (`normal_completion`, mode `full_budget_within_max_pages`). Major+ backlog total `0`; URLs queued but not visited `7`; page capture budget `2`. Major+ threshold **(off — breadth crawl)** — breadth: **`--breadth-crawl`** (alias **`--stop-disable`**).

```json
{
  "crawlMode": "full_budget_within_max_pages",
  "stopReason": "normal_completion",
  "majorPlusFindingCountTotal": 0,
  "queuedRemainingAtStop": 7,
  "pagesCaptured": 2,
  "stopAfterMajorPlus": null,
  "stopAfterBacklog": null,
  "backlogFindingCountTotal": 42,
  ...
  "qualityGateViolationUnitsTotal": 31,
...
- **Overall score:** **38** / 100 (schema **v2**) — logarithmic pillar penalties + harmonic blend over **only** URLs that appear above in the crawl tables.
...
| `depthAndTechnicalDisclosure` | 12 | 352 | 26 |
| `trustAndEcosystemTruth` | 100 | 0 | 0 |
| `visualRhythmFirstScreen` | 100 | 0 | 0 |
| `accessibilitySemanticsMeta` | 100 | 0 | 0 |
| `visualCatalogGovernance` | 20 | 200 | 16 |
...
|---|---:|---:|---:|---:|---:|
| http://127.0.0.1:60517/ | 20 | 0 | 21 | 94 | 4 |
| http://127.0.0.1:60517/index.html | 20 | 0 | 21 | 94 | 4 |

Findings rollup: **minor: 6**

## Homepage / first analyzed URL — first-screen snapshot

- URL: http://127.0.0.1:60517/
- H1: "Govern workspace delivery with traceable evidence"
- Above-fold words: 280
- CTA-like items above fold: "Get started", "View docs"
...
|----------|--------|------|-----|---------|
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces. |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | Template SVG assets/svg/template-bullet-chart.svg for "bullet" matches only 1/2 required catalog legend node labels (of  |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | Template SVG assets/svg/template-pie-donut.svg for "pie" matches only 0/2 required catalog legend node labels (of 4 lege |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | Template SVG assets/svg/template-radar.svg for "radar" matches only 1/2 required catalog legend node labels (of 3 legend |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | Template SVG assets/svg/template-waterfall.svg for "waterfall" matches only 2/3 required catalog legend node labels (of  |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces. |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #6b7280 is used outside the theme token allowlist; prefer var(--forge-*) or other sanctioned design tokens |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #2563eb is used outside the theme token allowlist; prefer var(--forge-*) or other sanctioned design tokens |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #b91c1c is used outside the theme token allowlist; prefer var(--forge-*) or other sanctioned design tokens |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #a16207 is used outside the theme token allowlist; prefer var(--forge-*) or other sanctioned design tokens |


### Priority detail

- **WARN / visual-catalog** (http://127.0.0.1:60517/): KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces.
  - Evidence: instanceCountByHash[Msm]=2 in DOM scan
  - Remediation: If multiple instances are unintended duplicates, dedupe visual roots; otherwise document repeated use in the catalog contract.
- **WARN / visual-catalog** (http://127.0.0.1:60517/): Template SVG assets/svg/template-bullet-chart.svg for "bullet" matches only 1/2 required catalog legend node labels (of 4 legend entries).
  - Evidence: diagram-labels-legend-gap key=bullet svg=assets/svg/template-bullet-chart.svg matched=1 required=2 url=http://127.0.0.1:60517/
...
- **WARN / visual-catalog** (http://127.0.0.1:60517/): Template SVG assets/svg/template-pie-donut.svg for "pie" matches only 0/2 required catalog legend node labels (of 4 legend entries).
  - Evidence: diagram-labels-legend-gap key=pie svg=assets/svg/template-pie-donut.svg matched=0 required=2 url=http://127.0.0.1:60517/
  - Remediation: Align SVG text nodes with catalog legend `items[].node` strings for the data-diagram-key, or update the catalog legend when labels were intentionally renamed.
- **WARN / visual-catalog** (http://127.0.0.1:60517/): Template SVG assets/svg/template-radar.svg for "radar" matches only 1/2 required catalog legend node labels (of 3 legend entries).
  - Evidence: diagram-labels-legend-gap key=radar svg=assets/svg/template-radar.svg matched=1 required=2 url=http://127.0.0.1:60517/
  - Remediation: Align SVG text nodes with catalog legend `items[].node` strings for the data-diagram-key, or update the catalog legend when labels were intentionally renamed.
- **WARN / visual-catalog** (http://127.0.0.1:60517/): Template SVG assets/svg/template-waterfall.svg for "waterfall" matches only 2/3 required catalog legend node labels (of 5 legend entries).
  - Evidence: diagram-labels-legend-gap key=waterfall svg=assets/svg/template-waterfall.svg matched=2 required=3 url=http://127.0.0.1:60517/
  - Remediation: Align SVG text nodes with catalog legend `items[].node` strings for the data-diagram-key, or update the catalog legend when labels were intentionally renamed.
- **WARN / visual-catalog** (http://127.0.0.1:60517/): KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces.
  - Evidence: instanceCountByHash[Msm]=2 in DOM scan
  - Remediation: If multiple instances are unintended duplicates, dedupe visual roots; otherwise document repeated use in the catalog contract.
- **WARN / readability** (http://127.0.0.1:60517/): Raw hex color #6b7280 is used outside the theme token allowlist; prefer var(--forge-*) or other sanctioned design tokens.
...
- **WARN / readability** (http://127.0.0.1:60517/): Raw hex color #2563eb is used outside the theme token allowlist; prefer var(--forge-*) or other sanctioned design tokens.
  ...
  - Remediation: Replace raw hex literals with theme custom properties (e.g. var(--forge-cyan)) or add the value to a theme pack token definition instead of feature CSS.
- **WARN / readability** (http://127.0.0.1:60517/): Raw hex color #b91c1c is used outside the theme token allowlist; prefer var(--forge-*) or other sanctioned design tokens.
  ...
  - Remediation: Replace raw hex literals with theme custom properties (e.g. var(--forge-cyan)) or add the value to a theme pack token definition instead of feature CSS.
- **WARN / readability** (http://127.0.0.1:60517/): Raw hex color #a16207 is used outside the theme token allowlist; prefer var(--forge-*) or other sanctioned design tokens.
...
## All findings this run (42)

| Severity | Legacy | Area | URL | Finding | Evidence (trim) |
|----------|--------|------|-----|---------|----------------|
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces. | instanceCountByHash[Msm]=2 in DOM scan |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | Template SVG assets/svg/template-bullet-chart.svg for "bullet" matches only 1/2 required catalog leg | diagram-labels-legend-gap key=bullet svg=assets/svg/template-bullet-chart.svg ma |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | Template SVG assets/svg/template-pie-donut.svg for "pie" matches only 0/2 required catalog legend no | diagram-labels-legend-gap key=pie svg=assets/svg/template-pie-donut.svg matched= |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | Template SVG assets/svg/template-radar.svg for "radar" matches only 1/2 required catalog legend node | diagram-labels-legend-gap key=radar svg=assets/svg/template-radar.svg matched=1  |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | Template SVG assets/svg/template-waterfall.svg for "waterfall" matches only 2/3 required catalog leg | diagram-labels-legend-gap key=waterfall svg=assets/svg/template-waterfall.svg ma |
| warn | medium | visual-catalog | http://127.0.0.1:60517/ | KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces. | instanceCountByHash[Msm]=2 in DOM scan |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #6b7280 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".st-queued" p |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #2563eb is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".st-running"  |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #b91c1c is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".st-failed" p |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #a16207 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".st-cancelled |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #1e293b is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #334155 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #1e293b is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #334155 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #334155 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".fleet-tile-- |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #e2e8f0 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #1e293b is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/ | Raw hex color #334155 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | visual-catalog | http://127.0.0.1:60517/index.html | KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces. | instanceCountByHash[Msm]=2 in DOM scan |
| warn | medium | visual-catalog | http://127.0.0.1:60517/index.html | Template SVG assets/svg/template-bullet-chart.svg for "bullet" matches only 1/2 required catalog leg | diagram-labels-legend-gap key=bullet svg=assets/svg/template-bullet-chart.svg ma |
| warn | medium | visual-catalog | http://127.0.0.1:60517/index.html | Template SVG assets/svg/template-pie-donut.svg for "pie" matches only 0/2 required catalog legend no | diagram-labels-legend-gap key=pie svg=assets/svg/template-pie-donut.svg matched= |
| warn | medium | visual-catalog | http://127.0.0.1:60517/index.html | Template SVG assets/svg/template-radar.svg for "radar" matches only 1/2 required catalog legend node | diagram-labels-legend-gap key=radar svg=assets/svg/template-radar.svg matched=1  |
| warn | medium | visual-catalog | http://127.0.0.1:60517/index.html | Template SVG assets/svg/template-waterfall.svg for "waterfall" matches only 2/3 required catalog leg | diagram-labels-legend-gap key=waterfall svg=assets/svg/template-waterfall.svg ma |
| warn | medium | visual-catalog | http://127.0.0.1:60517/index.html | KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces. | instanceCountByHash[Msm]=2 in DOM scan |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #6b7280 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".st-queued" p |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #2563eb is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".st-running"  |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #b91c1c is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".st-failed" p |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #a16207 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".st-cancelled |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #1e293b is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #334155 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #1e293b is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #334155 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #334155 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector=".fleet-tile-- |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #e2e8f0 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #1e293b is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| warn | medium | readability | http://127.0.0.1:60517/index.html | Raw hex color #334155 is used outside the theme token allowlist; prefer var(--forge-*) or other sanc | raw_hex_outside_allowlist path=css/forge-fleet-admin.css selector="[data-bs-them |
| minor | medium | readability | http://127.0.0.1:60517/ | Measured paragraph width suggests very long line lengths (readability / enterprise polish). | max_paragraph_measure_px=1184 |
| minor | medium | visual-catalog | http://127.0.0.1:60517/ | generator/bootstrap_missing_rule_pages.py inlines governed KS hash attributes (/data-ks-hash\s*=\s*[ | python_source=generator/bootstrap_missing_rule_pages.py kind=manual-literal |
| minor | medium | visual-catalog | http://127.0.0.1:60517/ | generator/build_rule_defect_fixtures.py inlines governed KS hash attributes (/data-ks-hash\s*=\s*["' | python_source=generator/build_rule_defect_fixtures.py kind=manual-literal |
| minor | medium | readability | http://127.0.0.1:60517/index.html | Measured paragraph width suggests very long line lengths (readability / enterprise polish). | max_paragraph_measure_px=1184 |
| minor | medium | visual-catalog | http://127.0.0.1:60517/index.html | generator/bootstrap_missing_rule_pages.py inlines governed KS hash attributes (/data-ks-hash\s*=\s*[ | python_source=generator/bootstrap_missing_rule_pages.py kind=manual-literal |
| minor | medium | visual-catalog | http://127.0.0.1:60517/index.html | generator/build_rule_defect_fixtures.py inlines governed KS hash attributes (/data-ks-hash\s*=\s*["' | python_source=generator/build_rule_defect_fixtures.py kind=manual-literal |
...
- screenshots/01-127-0-0-1-60517.png
- screenshots/00-mobile-127-0-0-1-60517.png
- screenshots/02-127-0-0-1-60517-index-html.png
