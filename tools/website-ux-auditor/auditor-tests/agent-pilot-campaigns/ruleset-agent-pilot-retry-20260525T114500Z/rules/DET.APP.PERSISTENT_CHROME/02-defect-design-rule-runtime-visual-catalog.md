...
- Cluster coverage share (findings in this cluster / all findings): 33.3%.
...
- `visualCatalogGovernance`: rawDamage 278.00 (14 finding(s))
...
- **WARN** Template SVG assets/svg/template-waterfall.svg for "waterfall" matches only 2/3 required catalog legend node labels (of 5 legend entries). (n/a)
  ...
  - Remediation hint: Align SVG text nodes with catalog legend `items[].node` strings for the data-diagram-key, or update the catalog legend when labels were intentionally renamed.
- **WARN** KS visual hash Msm appears on 2 elements — verify this is intentional for reusable surfaces. (n/a)
  - Evidence: instanceCountByHash[Msm]=2 in DOM scan
  - Remediation hint: If multiple instances are unintended duplicates, dedupe visual roots; otherwise document repeated use in the catalog contract.
- **WARN** Template SVG assets/svg/template-bullet-chart.svg for "bullet" matches only 1/2 required catalog legend node labels (of 4 legend entries). (n/a)
  - Evidence: diagram-labels-legend-gap key=bullet svg=assets/svg/template-bullet-chart.svg matched=1 required=2 url=http://127.0.0.1:60517/index.html
  - Remediation hint: Align SVG text nodes with catalog legend `items[].node` strings for the data-diagram-key, or update the catalog legend when labels were intentionally renamed.
...
- `tools/website-ux-auditor/auditor-tests/agent-pilot-campaigns/ruleset-agent-pilot-retry-20260525T114500Z/rules/DET.APP.PERSISTENT_CHROME/rca-prompts/8f895c64766e014e-f00.md` · warn · http://127.0.0.1:60517/
