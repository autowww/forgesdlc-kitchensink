#!/usr/bin/env bash
# DET ruleset harness: defect fixture pages + remediation loop + detection gate.
#
# Usage:
#   ./invoke-det-ruleset-harness.sh [options]
#
# Options:
...
#   --no-watch
#   --enable-agents         Run remediation agent when findings > 0
#   --enable-meta-agent     Meta-agent on remediation failure
#
# Default: detection-only (SKIP_CURSOR_AGENT=1), agents off.
...
log() {
  local msg="[det-harness $(date -u +%H:%M:%S)] $*"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-watch) NO_WATCH=1 ;;
    --enable-agents) ENABLE_AGENTS=1 ;;
    --enable-meta-agent) ENABLE_META_AGENT=1 ;;
    -h|--help)
      sed -n '1,22p' "$0"
      exit 0
      ;;
    *) echo "det-harness: unknown arg: $1" >&2; exit 2 ;;
  esac
  shift
done

if ! command -v jq >/dev/null 2>&1; then
  echo "det-harness: jq required" >&2
  exit 2
fi

if [[ ! -x "${LOOP}" ]]; then
  echo "det-harness: missing ${LOOP}" >&2
  exit 2
fi
...
if [[ "${HARNESS_WATCH}" -eq 1 ]]; then
  ...
  harness_merge "{\"phase\":\"harness_start\",\"mode\":\"$([[ ${ENABLE_AGENTS} -eq 1 ]] && echo agents || echo check-only)\",\"fixtureRoot\":\"${FIXTURE_ROOT}\",\"rulesTotal\":0,\"rulesDone\":0}"
  if [[ -t 2 ]]; then
...
for rule_id in "${REMAINING[@]}"; do
  ...
  [[ "${ENABLE_AGENTS}" -eq 1 ]] && mode_label="agents"
...
STUB
  ...
  if [[ "${ENABLE_AGENTS}" -eq 0 ]]; then
    loop_extra+=(--audit-only)
  fi
  ...
  fc="$(bash "${EXPECT}" "${audit_data}" "${rule_id}")"
  fc="${fc:-0}"

  log "${rule_id}: detection findings=${fc} loop_rc=${loop_rc}"
  ...
  if [[ "${fc}" -lt 1 ]]; then
  ...
  elif [[ "${ENABLE_AGENTS}" -eq 1 ]]; then
    harness_merge "$(jq -nc --arg rid "${rule_id}" '{phase:"remediation",currentRule:{ruleId:$rid,step:"agent"}}')"
    agent_attempts=1
    agent_extra=()
    for _a in "${loop_extra[@]}"; do
      [[ "${_a}" == "--audit-only" ]] && continue
      agent_extra+=("${_a}")
    done
    set +e
    bash "${LOOP}" "${LOOP_REPO}" "${rule_fixture}" "${agent_extra[@]}"
    agent_rc=$?
    set -e
    fc="$(bash "${EXPECT}" "${audit_data}" "${rule_id}")"
    fc="${fc:-0}"
    if [[ "${fc}" -eq 0 ]]; then
      ...
      note="findings remain after agent"
      write_remediation_failure "${rule_dir}" "${rule_id}" "${fc}" "${agent_attempts}"
      if [[ "${ENABLE_META_AGENT}" -eq 1 && -f "${TESTS_ROOT}/cursor-agent-run-learn-101-meta.sh" ]]; then
        log "${rule_id}: meta-agent (optional)"
        # shellcheck disable=SC1090
        true
      fi
    fi
    log "${rule_id}: post-agent findings=${fc} status=${final_status}"
  fi
  ...
  append_state "$(jq -nc \
    ...
    --arg m "${mode_label}" \
    --arg n "${note}" \
    '{ruleId:$r,lane:"deterministic",status:$s,findingsCount:$fc,agentAttempts:$aa,fixtureRoot:$fr,mode:$m,note:$n,ts:$t}')"
  ...
  if [[ "${final_status}" == "detection_miss" || "${final_status}" == "remediation_fail" ]]; then
    fin_rc=1
  fi
done
