...
7. **Repeat** until a full **live** crawl records **zero** blocker, critical, or major findings **and** the crawl queue is empty at your chosen budget (or you accept orphaned pages with a tracked exception list). The remediation loop runs an AI-assisted pass by default after this deterministic-clean state (**`FORGE_UX_ENABLE_AI_AUDIT`** defaults to **on**; set **`FORGE_UX_ENABLE_AI_AUDIT=0`** to skip); treat its artifacts as advisory follow-up, not as a replacement for deterministic sign-off.
