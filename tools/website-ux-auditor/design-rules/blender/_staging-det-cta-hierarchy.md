...
This deterministic rule (`collectCtaHierarchyReport` in `det-cta-hierarchy.check.js`) scans **logical viewport regions** and counts **primary** action controls—`<a>`, `<button>`, `<input type="button|submit|reset">`, and `[role="button"]` whose classes include **`btn-forge`** or **`btn-primary`**. Outline and low-emphasis styles (`btn-forge-outline`, `btn-cyan-outline`, `btn-outline`, `btn-secondary`, `btn-link`) are **not** primaries. The cap is **one primary per region** (`MAX_PRIMARY_CTAS_PER_REGION = 1`). Regions include:
...
Navigation, pagination, breadcrumbs, dropdown menus, tab lists, and cookie banners are excluded from region scans. Nested hero/modal roots are deduped so a child band does not double-count primaries already attributed to an outer region. Studio app workspaces use **`DET.APP.PRIMARY_CTA`** (`data-studio-primary-cta`) instead of this marketing-hero scan.
...
- `DET.APP.PRIMARY_CTA` — one dominant primary inside `[data-studio-workspace]` (app scope, not marketing heroes).
