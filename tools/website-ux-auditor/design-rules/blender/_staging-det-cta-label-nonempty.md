...
page_version: 6662056ba43b13b2fbe508c0f898f4c171b8e5a1ce205275c76c5c224735e8b8
...
- Icon-only controls (mobile offcanvas opener, toolbar glyphs) set **`aria-label="Open navigation"`** (or equivalent) on the **`<button class="btn btn-forge …">`** itself (`components/layouts.py`).
- Card footers use **`<a class="btn btn-forge" href="…">Read the guide</a>`** or **`btn btn-forge-outline`** with non-empty link text—not bare icons inside **`.forge-card`** footers.
...
- Icon-only **`<button class="btn btn-forge">`** containing only a decorative glyph span or SVG with **`aria-hidden="true"`**, with no `aria-label` and no `.sr-only` helper inside the control.
...
- Evidence strings like `empty_accessible_name tag="a"="a.btn-forge"` or `tag="button"="button.btn-forge[@Ldg]"` with **major** severity.
...
- `DET.HTML.EMPTY_INLINE` — empty inline semantics on non-interactive elements (independent of CTA naming).
