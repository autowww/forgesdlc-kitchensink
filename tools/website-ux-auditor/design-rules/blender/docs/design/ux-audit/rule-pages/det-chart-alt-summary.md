...
page_version: 6361e73cea465ffc4414d4215c2557eb97de1755dd17fe38c92bde90dac44cf7
...
This deterministic rule (`design-rules/deterministic/generated/det-chart-alt-summary.check.js`, metrics phase) scans visible chart roots (`[data-ks-chart]`, `.ks-chart-mount`, `[data-chart]`, and chart-class containers) and requires at least **8 characters** (`MIN_CHART_SUMMARY_CHARS`) of summary text via `aria-label`, `aria-labelledby`, `aria-describedby` (with a resolvable target), `title`, sr-only / `[data-chart-summary]` nodes, or a nearby `figcaption` / `p.forge-support` caption. Decorative ambient canvases inside `.forge-ambient-bg`, `.ks-ambient-bg`, `.forge-aurora`, and `aria-hidden` subtrees are excluded.
...
- Mount uses **`aria-describedby`** (or `aria-labelledby`) pointing at an element whose text is at least 8 characters—commonly `p.forge-support` copy within two sibling positions of the mount.
- **`figure` + `figcaption`** wraps the mount with a non-trivial caption, or **`p.forge-support`** / `[data-chart-summary]` sits adjacent (matches `render_ks_chart_mount` title line plus an insight sentence).
...
- Summary text is **consistent** with visible series labels where both exist (does not contradict bar labels or `.lenses-overview-donut-legend` swatches).
...
- **Title line omitted:** consumer pages leave a bare `<div class="ks-chart-mount mb-3" data-ks-chart>` with no `render_ks_chart_mount` caption and no insight line.
- **Tooltip-only** `<title>` inside SVG with no mount-level description (per-segment tooltips are not a substitute for page-level summary wiring).
...
Alternative passing pattern using `figure` / `figcaption` for API-driven mounts (`Dca`):
...
Pilot remediation may use the diagram-alt patcher registered for this rule id in `ux-deterministic-fixers/pilot-registry.json`.
