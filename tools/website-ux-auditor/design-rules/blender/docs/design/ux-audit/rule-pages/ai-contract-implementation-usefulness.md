...
page_version: 99b703dc72441894f1d45e352f46f202a213b7e35f6ca5744d277ec3683b1d12
generated_at: 2026-05-28T17:05:00.000Z
...
- **Expected look** and **Anatomy** name real KS classes and regions (for example `.ks-doc-breadcrumb`, `forge-card breathe-link`, `bento-grid`, `glass forge-stat`)—not "looks professional" without selectors.
- **States** cover what implementers must ship: default, interactive, empty, loading/error (when relevant), and reduced-motion coordination with related hashes (`Ksj` / `Ksc`).
- **Responsive behavior** states collapse, truncation, or wrap rules (for example deep breadcrumb paths use ellipsis + `title`, not horizontal scroll).
...
- **Implementation notes** point to real entrypoints (`components/layouts.py`, `render_breadcrumbs`, `chrome_region_attrs`, `generator/pages/index.py`) and hash emitters.
- **Forbidden patterns** are specific to the surface (not copied verbatim across unrelated hashes).
- Showcase or consumer HTML matches the contract's `root_selector` and hash markers without contradictory structure.
...
Passing KS markup: matches **Kbc — Doc breadcrumb** contract—`nav.ks-doc-breadcrumb` root with hash markers, Bootstrap `breadcrumb` list, terminal item `aria-current="page"` (from `_showcase_header` + `chrome_region_attrs("doc-breadcrumb")` in `components/layouts.py`).
    ...
    <div class="col-lg-9 col-xl-10 site-header-content">
      ...
      >
        <ol class="breadcrumb mb-1" style="font-size:0.75rem">
          <li class="breadcrumb-item"><a href="/showcase/" class="text-cyan" style="text-decoration:none">Showcase</a></li>
          <li class="breadcrumb-item"><a href="/showcase/layouts.html" class="text-cyan" style="text-decoration:none">Layouts</a></li>
          <li class="breadcrumb-item active text-dim" aria-current="page">Handbook chapter</li>
        </ol>
      </nav>
...
- `AI.PY.HTML_AUTHORING_QUALITY` — Python emitters must match contract anatomy and hash attrs.
- `AI.APP.PRIMITIVE_CONSISTENCY` — React primitive family contracts must specify states implementers can follow.
