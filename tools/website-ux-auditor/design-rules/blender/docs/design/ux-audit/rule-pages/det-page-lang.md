...
page_version: 92d1474ee3f383e7749ee80437fcdb2654c380980a8a8e89c662ac745e0c8d7e
...
This deterministic rule runs in the **metrics** phase. The crawler reads `document.documentElement.getAttribute('lang')` into `metrics.lang`. **`lang.check.js` passes when `metrics.lang` is truthy** (any non-empty BCP 47 tag, e.g. `en`, `en-US`). A page **fails** when `lang` is missing or empty.
...
Failing KS markup: handbook-style shell mirrors `handbook_page` anatomy (`forge-aurora`, `skip-link`, `forge-sidebar`, `nav-scroll`, `nav-section-label`, `nav-link`, `main#main`, `doc-content`, `section-label`, `font-display`, `forge-support`, `forge-card`, `card-label`, `container-fluid`) but the root omits `lang`, so assistive tech has no document default.
...
**Kitchen Sink:** Pass `html_lang="en"` (or the handbook locale) into `handbook_page(..., html_lang=...)`. Confirm `generator/build-showcase.py` and consumer builds emit `lang` on the document element (see `components/layouts.py` `<html lang="{e(html_lang)}">`).
...
**Harness:** `generator/build_rule_defect_fixtures.py` and `auditor-tests/invoke-det-ruleset-harness.sh --only-rule DET.PAGE.LANG` for regression. Implementation: `tools/website-ux-auditor/design-rules/deterministic/page/lang.check.js`. Deterministic fixer: `patch_page_lang` in `lib/ux-deterministic-fixers/fixers/patches/page-meta.mjs`.
