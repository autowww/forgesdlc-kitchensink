...
page_version: bc55bb8357237c518f70fb3ed0e6f0926c99c4068686fe8e8867197e5f1c7d6a
generated_at: 2026-05-29T14:32:00.000Z
...
This deterministic rule evaluates **`metrics.ksVisualHashReport`** from the UX auditor crawl (`lib/dom-metrics.js` → `lib/visual-catalog.js`). It flags invalid token shapes, attribute mismatches, one-sided markers, and (as a warning) repeated hashes on multiple nodes. **Registry membership** is **`DET.HASH.REGISTRY_ROW`**; contract file presence is **`DET.CONTRACT.PATH`**. Python emitters should use `ks_hash_attrs()` (`components/ks_hash_attrs.py`) and registry helpers in `components/ks_catalog_hashes.py` so builds fail before shipping one-sided markers.

**Plan:** Allocate or reuse a hash when adding a visual surface. **Do:** Emit both attributes from `ks_hash_attrs`, `layout_shell_attrs`, `chrome_region_attrs`, or React `ksReactPrimitiveAttrs` / `ksVisualAttrs`. **Check:** `node tools/website-ux-auditor/analyze-website-ux.mjs` on built `showcase/` or consumer `website/`, or grep HTML for `[data-ks-hash]` and confirm each node has a matching `hash=`. **Adjust:** Fix pairing before chasing registry or contract issues — a page can pass **`DET.HASH.MARKERS`** while **`DET.CONTRACT.PATH`** still fails.
...
- Consumer sites that submodule KS reproduce the same marker pattern on handbook shells (**Hbk**, **Ksr**, **Kco**, **Ksf**) and doc pages (**Ctr**, **Fag**) after `python3 generator/build-showcase.py` / site generators run.
...
- **`incompleteMarkers` (warn):** Only `data-ks-hash` without `hash=`, or only `hash=` without `data-ks-hash` on a governed root (`side: hash-missing` or `data-ks-hash-missing` in DOM metrics).
...
- `DET.APP.PRIMITIVE_MARKERS` — React primitives emit governed `data-ks-type` / `data-ks-name` / hash trio.
