...
page_version: 6e6aedec08a6c94657b5f8527df68098326e850992fa632d5c52fb6e9b7939f3
generated_at: 2026-05-29T16:00:00.000Z
...
1. **`duplicate-destination`** — the same pathname appears in **conflicting peer bands**: primary ↔ sidebar, primary ↔ offcanvas, or sidebar ↔ offcanvas (`CONFLICTING_BAND_PAIRS` in `det-nav-dedup.check.js`).
2. **`duplicate-primary-roots`** — two distinct primary nav roots (for example nested `header.site-header nav` trees plus **`Kpn`**) link to the same destination, violating the single masthead horizon in the **Kpn** contract.

**Breadcrumb hierarchy is intentional:** when **`Kbc`** (`nav.ks-doc-breadcrumb`) repeats a destination that also appears in exactly one other chrome band, the duplicate is exempt — the trail orients; the rail navigates. Footer, ToC (**`Ktx`**), and breadcrumb bands are not conflicting peers with primary/sidebar/offcanvas in this check. Skip links, theme toggles, and cookie/consent anchors are ignored.
...
- **`Kbc`** may repeat a parent segment (for example `/docs`) while **`Ksr`** holds section hrefs — auditor treats breadcrumb + one chrome band as intentional hierarchy.
...
- Footer utility links (`footer nav`, `.ks-site-footer-region nav`) and **`Ktx`** in-page anchors may repeat destinations without triggering peer-band conflicts (not in `CONFLICTING_BAND_PAIRS`).
- Auxiliary controls excluded: skip-to-content, `#main` fragments, theme switchers, and consent links do not pollute the graph.

## Failing signals

- **`duplicate-destination`** with `bands=primary+sidebar` (or `primary+offcanvas`, `sidebar+offcanvas`) — same normalized pathname (for example `/docs/start`) and visible labels like “Getting started” in both **`Kpn`** / `header.site-header nav` and **`Ksr`**.
...
Failing KS markup: handbook-style shell mirrors `/docs/start` in both `header.site-header nav` and **`Ksr`** sidebar — conflicting primary + sidebar bands without breadcrumb exemption.
...
1. **Split IA by band** — move handbook section links exclusively into **`Ksr`**; keep **`Kpn`** / `header.site-header nav` to curated site-level destinations that do not repeat sidebar paths.
