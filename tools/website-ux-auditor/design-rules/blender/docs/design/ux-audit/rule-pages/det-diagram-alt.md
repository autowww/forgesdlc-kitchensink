...
page_version: 12d4a2e16a8c8ecdb3d36c2b5e52387424279bc1bc966948d369d91e703331e2
generated_at: 2026-05-29T14:22:00.000Z
...
Kitchen Sink ships mechanism diagrams as static tiles (`ks_diagram_tile_html`, `forge-diagram`, `ks-diagram-tile`, `ks-diagram-canvas`), ASCII figures (`figure.forge-diagram-ascii`), and catalog-linked mounts (`data-diagram-key`, expandable `forge-diagram-trigger` / `ks-diagram-trigger`). Screen readers need a **consistent contract**: decorative assets stay out of the accessibility tree; informative diagrams expose a name through `alt`, `aria-label`, `aria-labelledby`, `aria-describedby`, or nearby `figcaption` / `forge-support` copy (minimum **3 characters**, `MIN_DIAGRAM_ALT_CHARS`).
...
- **Figure + caption:** `figure.forge-diagram` or `figure.forge-diagram-ascii` with **`figcaption.forge-diagram-ascii-caption`** or `forge-support` copy at least 3 characters, satisfying the name requirement without a misleading empty `alt`.
...
- **ASCII diagram:** `figure.forge-diagram.forge-diagram-ascii` with `data-diagram-key` and `aria-label` or `figcaption` when informative; decorative ASCII uses `role="presentation"` per `ascii_diagram_figure_html` decorative mode.
...
Failing KS markup: informative tile hidden from AT (`diagram-alt-informative-hidden`), `aria-hidden` with `role="img"` (`diagram-alt-conflicting-role`), catalog tile without a name (`diagram-alt-missing-summary`), and empty `alt` with parent `aria-label` (`diagram-alt-decorative-named`).
...
Passing KS markup aligned with `ks_diagram_tile_html`: informative catalog tile with `role="figure"`, non-empty `alt`, and visible `forge-support` caption; decorative companion uses `role="presentation"` with empty `alt` and `aria-hidden` only.
...
Passing ASCII diagram (`ascii_diagram_figure_html` informative shape):

```html
<figure
  class="forge-diagram forge-diagram-ascii breathe-static mb-4"
  data-diagram-key="gate"
  role="figure"
  aria-label="ASCII gate chain: Stage 1 through Stage 3 with Gate 1 and Gate 2"
  hash="Asc"
  data-ks-hash="Asc"
>
  <div class="forge-diagram-ascii-canvas">
    <pre class="forge-code forge-diagram-ascii-pre"><code class="language-text">[ Intent ] --&gt; [ Review ] --&gt; [ Agent ] --&gt; [ Release ]</code></pre>
  </div>
