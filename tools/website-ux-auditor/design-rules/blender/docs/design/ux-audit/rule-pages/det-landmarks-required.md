...
page_version: 58d4fac004b41f75b66bee1146ff01cb4a7df42331b22972a5b4e63167cb9b3b
generated_at: 2026-05-29T14:30:00.000Z
...
- **`missing-main`:** Primary content sits in **`div.doc-main`**, **`div.fs-landing-main`**, **`article`**, or a Bootstrap column with no `main` / `[role="main"]` landmark (`main_landmark_count=0`).
...
- **`missing-nav`:** **`header.site-header`**, **`header.landing-header`**, or header chrome exposes ≥ **2** global links (`Overview`, `Docs`, `Trust`, …) but links are bare anchors with no wrapping **`nav`** outside `main` (`nav_applicable=true`, `nav_landmark_count=0`).
...
Passing KS markup: same handbook anatomy with semantic landmarks — **`a.skip-link`** targeting **`#main`**, **`header.site-header`**, **`nav.nav-scroll`** in the sidebar rail, **`main#main.doc-main`**, and **`footer.ks-site-footer-region`** (**Ksf**) outside the content column.
...
2. **Landmark the global nav** — when ≥ **2** wayfinding links live outside `main`, wrap them in `<nav aria-label="…">` in **`header.landing-header`** with **`nav.landing-header-navbar`**, **`aside.forge-sidebar`**, or a dedicated shell rail; do not rely on bare anchor lists in header chrome.
