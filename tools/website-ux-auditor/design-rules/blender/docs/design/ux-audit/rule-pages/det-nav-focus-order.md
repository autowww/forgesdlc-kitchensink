...
page_version: a6c89528e5e1f4d1066101500bd5b91290538c8fc03eaa2c8ca104d9f043f579
...
| **`handbook-chrome-tab-suppressed`** | Curated handbook top nav is present and **> 16** sidebar / in-page ToC / breadcrumb links use **`tabindex="-1"`** (detected via `.fleet-handbook-topnav` and complementary selectors) |
...
- `DET.NAV.DEDUP` — removing duplicate nav bands avoids revisiting the same destinations mid-tab-path.
...
- `AI.JS.BEHAVIOR_DISCOVERABILITY` — scripts that move focus on hover or open panels must still expose a keyboard path consistent with visual affordances.
