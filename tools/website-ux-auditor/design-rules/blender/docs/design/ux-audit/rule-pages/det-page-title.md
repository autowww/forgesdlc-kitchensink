...
page_version: 2c3d7fb35ec29318f5827ffa47fff4e0134bd67a3a52c6444e7cbff4fea91acf
...
Failing KS markup: handbook-style shell mirrors `handbook_page` anatomy (`forge-aurora`, `forge-sidebar`, `nav-scroll`, `main#main`, `doc-content`, `font-display` heading) but leaves the scaffold placeholder **`Home`** in `<title>` instead of a descriptive handbook + chapter label.
...
**Harness:** `generator/build_rule_defect_fixtures.py` and `auditor-tests/invoke-det-ruleset-harness.sh --only-rule DET.PAGE.TITLE` for regression. Implementation: `design-rules/deterministic/page/title.check.js` (auditor root). Deterministic fixer adapter: `page_title` in `lib/ux-deterministic-fixers/`.
