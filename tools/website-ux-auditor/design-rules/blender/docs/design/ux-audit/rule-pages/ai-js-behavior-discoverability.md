...
page_version: 682e546ce56b8a85e9ce9add97e37dc5c05dc4e2fd4350af55b8fd119d17bbe6
...
Kitchen Sink ships **interaction scripts** under `js/` (family **`Ksj`**: nav/offcanvas, theme picker, diagram modals via `js/ks-diagram-catalog.js`, chart mounts in `forge-data-charts.js`, presentation fullscreen, roadmap edits, home tile expansion). Many behaviors are progressive enhancements on markup from `components/layouts.py` and transforms—not obvious from static HTML alone.

Deterministic gates (`DET.JS.PROGRESSIVE`, `DET.JS.NO_CONSOLE_ERROR`, `DET.NAV.FOCUS_ORDER`, `DET.MOTION.PREFERS_REDUCED`, `DET.APP.FOCUS_TRAP`) prove baseline degradation, console hygiene, focus order, motion prefs, and modal/offcanvas traps. This AI rule judges **discoverability**: can a visitor or operator infer what will happen before they click, tap, or hover—and find a keyboard/touch path when the script adds behavior?
...
- Theme switching uses `forge-theme-trigger` with `aria-label`, `title`, and visible `forge-theme-trigger__copy`—not icon-only mystery meat.
...
- Hover-only affordance (`forge-diagram-trigger::after` "Click to expand") with no focus-visible equivalent or persistent text for touch users.
...
4. Replace hover-only discovery with persistent text or focus-visible hints; verify keyboard path (`DET.NAV.FOCUS_ORDER`, `DET.APP.FOCUS_TRAP`).
...
- `DET.APP.FOCUS_TRAP` — visible openers and dismiss paths for Bootstrap modals and offcanvas shells.
