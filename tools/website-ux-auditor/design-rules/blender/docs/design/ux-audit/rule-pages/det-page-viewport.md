...
page_version: 9970ddaefb906abb69a9ca9f962f5fa4b130831974123c69db1abd344c1b230b
...
Without a **viewport meta tag**, mobile browsers assume a ~980px layout width. Kitchen Sink responsive grids (`container-fluid`, `col-lg-*`, `d-lg-flex`, `clamp()` headings) collapse incorrectly: `forge-sidebar` stays desktop-sized, `doc-content` shrinks to a narrow column, and users must pinch-zoom to read body copy.
...
`handbook_page`, `chapter_page`, `product_page`, `landing_page`, and showcase shells in `components/layouts.py` include this line in `<head>` immediately after charset and color-scheme init, alongside `<title>` and theme CSS.
...
- **`handbook_page`** and sibling layouts in `layouts.py` emit the tag immediately after charset / `FORGE_COLOR_SCHEME_INIT`.
...
- Mobile layout behaves as authored: `forge-sidebar` uses `d-none d-lg-flex`, `doc-content` uses readable width, Bootstrap breakpoints apply.
...
Failing KS markup: handbook-style shell mirrors `handbook_page` anatomy (`forge-aurora`, `forge-sidebar`, `nav-scroll`, `main#main`, `doc-content`) but omits viewport meta, so mobile browsers default to a desktop layout width.
...
**Harness:** `generator/build_rule_defect_fixtures.py` and `auditor-tests/invoke-det-ruleset-harness.sh --only-rule DET.PAGE.VIEWPORT` for regression. Implementation: `tools/website-ux-auditor/design-rules/deterministic/page/viewport.check.js`. Deterministic fixer: `patchPageViewport` in `lib/ux-deterministic-fixers/fixers/patch-registry.mjs`.
