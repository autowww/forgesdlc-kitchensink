...
page_version: 76312a81f40ff67f7c208188f1a03483576b4ede66afb02c84e1dcf3458d24d6
...
Kitchen Sink **`landing_page`** shells (`components/layouts.py`, `css/forgesdlc-theme.css`) are meant to lead with a product story: curated **`landing-header`** / **`landing-nav`**, a **`landing-hero`** band, and depth routes under `/docs` or `/handbook`. When a public homepage reuses handbook chrome—**`fs-primary-nav-global`**, persistent **`fs-sidebar`** or **`forge-sidebar`**, offcanvas trees, and wide link walls—the first screen reads as documentation before the visitor sees what the product does.

This deterministic rule enforces the Forge enterprise **first-screen budget** from `docs/design/forge-enterprise-ai-website-standard.md` using crawl metrics (`lib/dom-metrics.js`) and a Playwright hero-fold probe (`collectContextBurdenReport` in `det-context-burden.check.js`):
...
| First-viewport link count (handbook inner) | ≤ **24** | Platform handbook inner chapters (**warn**) |

Interactive controls count visible `<button>`, `[role="button"]`, `.btn` anchors, and submit/button inputs in the hero fold (top ~92% viewport height, capped at 900px), excluding pagination, dropdown menus, and in-nav controls.
...
- No persistent **`fs-sidebar`**, **`forge-sidebar`**, or generated handbook tree on desktop homepage view; full trees live under `/docs`, `/handbook`, or `/reference`.
...
- Handbook inner chapters may use higher link density in sidebar/ToC but should stay ≤ **24** first-viewport links to avoid link-wall fatigue.

## Failing signals

- **`preMainFirstH1LinkCount` > 10** — link walls, **`fs-cross-refs`**, or chrome links appear above the primary in-main headline (`pre_main_first_h1_link_count=N max=10`, **major**; **critical** when count exceeds max + 2).
- **`outsideMainHeaderNavLinkCount` > 7** — **`landing-nav`** or **`fs-primary-nav-global`** exposes too many top-level choices (`outside_main_header_nav_links=N max=7`; **critical** when count exceeds max + 4).
- **`navChromeContainerCount` > 4** — competing **`header`**, **`nav`**, **`aside`**, **`fs-sidebar`** / **`forge-sidebar`**, and offcanvas bands outside **`main`** (`nav_chrome_container_count=N max=4`).
...
- On platform handbook inner routes: **`firstViewportLinkCount` > 24** — sidebar, ToC, header, and early tables stack too many links above the fold (**warn**).
- Homepage screenshot reads as handbook reader despite hero copy — often co-fails **`DET.PAGE.MODE`** and **`DET.NAV.DEDUP`**.

## Before example

Failing KS markup: handbook-style chrome on `/` — stacked nav bands, **`fs-sidebar`** tree, pre-main cross-refs, and four hero buttons. Exceeds header nav, nav-band, pre-main, and hero-control budgets.
...
**Evidence:** Crawl metrics from `lib/dom-metrics.js` — `preMainFirstH1LinkCount`, `outsideMainHeaderNavLinkCount`, `navChromeContainerCount`, `firstViewportLinkCount` (home only; handbook inner chapters warn at 24). Playwright probe `contextBurdenReport.heroInteractiveCount` from `collectContextBurdenReport`. Finding messages cite keys such as `pre_main_first_h1_link_count=14 max=10`, `outside_main_header_nav_links=12 max=7`, `nav_chrome_container_count=6 max=4`, `first_viewport_link_count=30 max=28`, `handbook_first_viewport_link_count=26 max=24`, `hero_interactive_controls=5 max=3`.

**Remediate (in order):**

1. **Shell:** Route `/` through **`landing_page`**, not **`product_page`** with persistent **`fs-sidebar`** or **`forge-sidebar`**. Move handbook trees to `/docs` / `/handbook` deep routes.
