...
page_version: dea6bb3c09a5b7eb7caee8bdef5088d6572376d4879234f9daf370edc297dfff
generated_at: 2026-05-29T17:30:00.000Z
...
- **`competing-modes`** — detected modes include both **marketing** and **handbook** (for example `layout-landing` on the layout root plus six or more sidebar links in **`.forge-sidebar`** / **`aside[data-ks-name="doc-sidebar"]`** outside `main`).
...
Failing KS markup: public landing declares **`layout-landing`** (`Ldg`) but still mounts a handbook doc rail beside the hero—marketing and handbook modes compete above the fold.
...
Passing KS markup: same landing story with **`landing_page`** anatomy only—explicit mode meta, collapsible **`landing-header-navbar`**, hero band inside **`fs-landing-main`**, and body content in **`fs-landing-body-shell`**; handbook tree lives under `/docs` routes, not on `/`.
...
**Kitchen Sink:** Call **`landing_page`** for public roots (`components/layouts.py`); pass `use_collapsible_nav=True` when you need **`landing-header-navbar`** / `#fsLandingNav`. Use **`handbook_page`** / **`chapter_page`** only on handbook URLs. Confirm `layout_shell_attrs` resolves to `data-ks-name="layout-landing"` (registry slug **`layout-landing`**, hash **`Ldg`**). Rebuild showcase and consumer sites after layout changes.
...
- `DET.PAGE.TITLE` — non-empty, descriptive `<title>` after mode is declared.
