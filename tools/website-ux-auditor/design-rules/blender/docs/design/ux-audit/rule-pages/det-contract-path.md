...
page_version: a26fd0c9c0280afa1d7602dbaf99dda0c572f7e2fad633532b10463afc04607e
...
This deterministic rule scans `docs/design/catalog/visual-registry.generated.json` during the **metrics** phase (`scanContractPaths` in `det-contract-path.check.js`). It flags **`empty-path`** (required status but no path) and **`missing-file`** (path set but file absent). Rows with other statuses (for example **`not-applicable`**) are out of scope when policy does not require a contract file.
...
|--------|----------------|-------------|
| `empty-path` | UX audit `visual-catalog` finding; `contractPathReport.issues` | Set `contract:` to `docs/design/catalog/...` or change `contract_status` to `not-applicable` only with a justified family rule. |
| `missing-file` | Evidence `hash=Hbk contract=docs/design/catalog/layouts/Hbk-layout-handbook-OLD.md` | Restore the `.md` file or update `visual-registry.yaml` and regenerate `visual-registry.generated.json`. |
| Truncated pass | `contract_path_total=N` in evidence | `node tools/design-catalog/check-visual-catalog.mjs --repo .` lists every failure beyond the 12-finding cap. |
| No registry | `skipped: true, reason: no-registry` | Run `node tools/design-catalog/inventory-ks-visuals.mjs` and commit generated JSON before UX audit. |
...
node tools/website-ux-auditor/analyze-website-ux.mjs --repo . --site-root showcase
```

After fixing paths, clear **`DET.CONTRACT.PATH`** before treating **`DET.CONTRACT.PLACEHOLDERS`**, **`DET.CATALOG.CONTRACT_SPECIFICITY`**, or **`AI.CONTRACT.IMPLEMENTATION_USEFULNESS`** as authoritative for that hash.
