...
page_version: b7cc93206211ddd3f1dfbb848bad56ec5aec6bc6024f3c745c3c4a1245136f0f
...
Kitchen Sink landing and product heroes render conversion actions in **`landing-hero-actions`** with a **`landing-hero-actions__buttons`** flex row (`components/components.py`, `css/forgesdlc-theme.css`). When four or five filled buttons sit side by side on one horizontal baseline, scanners cannot pick a primary path and touch targets crowd on mobile.

This deterministic rule counts **visible horizontal action controls**—`<button>`, `input[type=button|submit|reset]`, `[role="button"]`, and `<a class="btn …">`—inside known group containers (`.landing-hero-actions`, `.landing-hero-actions__buttons`, `.btn-group`, `.btn-toolbar`, `[class*="hero-actions"]`, `[class*="cta-row"]`, `[class*="__actions"]`, `[role="group"]`, and flex-row clusters in `main`). Actions must align on one row (top spread ≤ 14px, left spread > 24px). The default cap is **three** (`MAX_VISIBLE_HORIZONTAL_ACTIONS`). Navigation, pagination, tab lists, dropdown menus, header nav, and cookie banners are excluded.
...
- **`metrics.buttonGroupMaxReport`** reports `maxAllowed: 3`, `groupCount: 0`, and `violations: []`.
...
- Page passes **`DET.CTA.HIERARCHY`** (one primary class) yet still fails here because **count**, not class role, is the gate.
...
**Evidence:** Auditor field **`metrics.buttonGroupMaxReport`** with `maxAllowed: 3` and `violations[]` entries (`kind: too-many-actions`, `actionCount`, `selectorHint`, `className`, `labels`). Findings cite `visible_actions=N max=3` and the offending group class. Reproduce in DevTools by selecting `.landing-hero-actions__buttons` and counting visible `.btn` / `button` siblings that share one horizontal baseline.
