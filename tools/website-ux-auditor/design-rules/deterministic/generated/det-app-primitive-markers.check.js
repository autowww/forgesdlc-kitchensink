/**
 * DET.APP.PRIMITIVE_MARKERS — KS React primitive roots emit data-ks-hash, data-ks-type,
 * and data-ks-name per ksReactPrimitiveAttrs / ksVisualAttrs conventions.
 */

/** Cap findings per page pass (DOM scan). */
export const MAX_APP_PRIMITIVE_MARKERS_FINDINGS = 10;
...
export const rule = {
  ...
  priorityWeight: 8,
  source: 'docs/design/ux-audit/deterministic-design-rules.md#det-app-primitive_markers',
};
 ...
 * @param {string} [url]
 */
export function findingsFromAppPrimitiveMarkersReport(report, url = '') {
  ...
  for (const v of violations.slice(0, MAX_APP_PRIMITIVE_MARKERS_FINDINGS)) {
  ...
  if (total > MAX_APP_PRIMITIVE_MARKERS_FINDINGS && findings.length >= MAX_APP_PRIMITIVE_MARKERS_FINDINGS) {
    findings.push({
      severity: 'minor',
      area: 'visual-catalog',
      message: `Additional react-primitive KS attribute issues omitted (${total - MAX_APP_PRIMITIVE_MARKERS_FINDINGS} more).`,
      evidence: `app_primitive_markers_violation_total=${total}`,
      remediation:
...
export async function collectAppPrimitiveMarkersReport(page) {
...
export async function run({ metrics, page, url }) {
  const report = metrics?.appPrimitiveMarkersReport
    ?? (page ? await collectAppPrimitiveMarkersReport(page) : null);
  if (!report || !(report.violations || []).length) return [];
  return findingsFromAppPrimitiveMarkersReport(report, url || metrics?.url || '');
}
