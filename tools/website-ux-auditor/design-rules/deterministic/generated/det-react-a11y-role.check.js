/**
 * DET.REACT.A11Y_ROLE — KS React primitives expose correct ARIA roles/states
...
export const rule = {
  ...
  priorityWeight: 8,
  source: 'docs/design/ux-audit/deterministic-design-rules.md#det-react-a11y_role',
};
 ...
 * @param {string} [url]
 */
export function findingsFromReactA11yRoleReport(report, url = '') {
...
export async function collectReactA11yRoleReport(page, minNameChars = MIN_ACCESSIBLE_NAME_CHARS) {
...
export async function run({ metrics, page, url }) {
  const report = metrics?.reactA11yRoleReport
    ?? (page ? await collectReactA11yRoleReport(page) : null);
  if (!report || !(report.violations || []).length) return [];
  return findingsFromReactA11yRoleReport(report, url || metrics?.url || '');
}
