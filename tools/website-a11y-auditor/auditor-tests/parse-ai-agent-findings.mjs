#!/usr/bin/env node
/**
 * Parse Cursor agent stdout into normalized AI findings JSON.
 */
import fs from 'node:fs/promises';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

import { extractJsonFromAgentText, normalizeAiFinding } from '../lib/ai-audit-batches.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

function usage() {
  console.error(
    'usage: node parse-ai-agent-findings.mjs --in TRANSCRIPT --out OUT.json [--rule-id AI.A11Y...]',
  );
}
