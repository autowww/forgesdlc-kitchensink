# Linear Circular Motion (`Mch`)

**Hash:** `Mch` · **Slug:** `linear-circular-motion` · **Showcase:** `#sec-linear-circular-motion`

Emitter: `components/spatial_wave2.py` or `components/spatial.py` · CSS: `css/ks-spatial-wave2.css`

## Purpose

Linear circular motion

## Expected behavior

Governed spatial primitive with hash marker, reduced-motion fallback, and preserve-3d support gate.

## Oracle scenarios

See [`../oracles/Mch.json`](../oracles/Mch.json).

## Accessibility

- Root emits `hash` and `data-ks-hash="Mch"`.
- Motion respects `prefers-reduced-motion: reduce`.
