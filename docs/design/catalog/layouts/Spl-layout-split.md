---
hash: "Spl"
name: "Split two-panel layout"
type: "layout"
status: "active"
source_paths:
  - components/layouts.py
showcase_url: "https://ks.forgesdlc.com/showcase/preview-split.html"
screenshot_url: "https://ks.forgesdlc.com/showcase/screenshots/Spl.png"
screenshot_status: "planned"
---

# Spl — Split two-panel layout
...
- **Showcase URL / status:** https://ks.forgesdlc.com/showcase/preview-split.html (active preview page)
- **Screenshot URL / status:** https://ks.forgesdlc.com/showcase/screenshots/Spl.png — status **planned**

## Purpose

Two-pane editorial or comparison layout for parallel scrolling columns or contrasting narratives.

## Expected look

- **Split two-panel** composition: primary pane plus secondary inspector/nav pane with clear resize/stack behavior.
- Neither panel reads like duplicated chrome—landmarks remain singular ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).

## Anatomy

Split columns with independent scroll where appropriate; shared header; balanced breakpoints.
...
- Showcase/build output honors **Spl** markers: emitted roots include both `hash="Spl"` and `data-ks-hash="Spl"` where `emit_marker_in_showcase` / museum rules apply.
...
- Does **Split two-panel layout** read as the correct *role* for consumers (handbook vs museum vs landing) rather than an accidental mash-up of two layouts?
...
Python `split_page` in `components/layouts.py`.
