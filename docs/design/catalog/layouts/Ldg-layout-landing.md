...
showcase_url: "https://ks.forgesdlc.com/showcase/preview-landing.html"
screenshot_url: "https://ks.forgesdlc.com/showcase/screenshots/Ldg.png"
...
- **Showcase URL / status:** https://ks.forgesdlc.com/showcase/preview-landing.html (active preview page)
- **Screenshot URL / status:** https://ks.forgesdlc.com/showcase/screenshots/Ldg.png — status **planned**

## Purpose

Public marketing landing: hero-first storytelling, outcome bands, and CTA rhythm suited to enterprise evaluation.

## Expected look

- **Landing** canvas: hero-forward vertical storytelling with generous bands; chrome stays minimal versus marketing body.
- Section stacks follow Forge landing rhythm (hero → proof → depth) without burying primary CTA ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).

## Anatomy

Full-width sections stack vertically; hero; card rails; flow or diagram bands; final CTA; footer.
...
- Showcase/build output honors **Ldg** markers: emitted roots include both `hash="Ldg"` and `data-ks-hash="Ldg"` where `emit_marker_in_showcase` / museum rules apply.
...
- Does **Landing layout** read as the correct *role* for consumers (handbook vs museum vs landing) rather than an accidental mash-up of two layouts?
...
Python `landing_page` in `components/layouts.py`; compose with `Kpr` marketing renderers.
