---
hash: "Rpl"
name: "React primitives live"
type: "page"
status: "active"
source_paths:
  - generator/pages/react_primitives_live.py
showcase_url: "https://ks.forgesdlc.com/cases/showcase/react-primitives-live.html"
screenshot_url: "https://ks.forgesdlc.com/cases/showcase/screenshots/Rpl.png"
screenshot_status: "planned"
---

# Rpl — React primitives live
...
- **Type:** page
- **Category:** page
- **Source paths:** `generator/pages/react_primitives_live.py`
- **Showcase URL / status:** https://ks.forgesdlc.com/cases/showcase/react-primitives-live.html (active preview page)
- **Screenshot URL / status:** https://ks.forgesdlc.com/cases/showcase/screenshots/Rpl.png — status **planned**

## Purpose

Showcase museum page **react-primitives-live** (`generator/pages/react-primitives-live.py`) documenting React primitives live patterns for KS maintainers and consumers.

## Expected look

- **Live primitives** scenarios exercise mounting, loading, and edge props—states visibly distinct.
- Density matches operator tooling; scrolling stays regional inside demos ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Rpl** markers: emitted roots include both `hash="Rpl"` and `data-ks-hash="Rpl"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `react_primitives_live.py` composes inside the advertised parent layout; built HTML for slug `react-primitives-live` includes the nested layout hash markers expected by showcase inventory.
...
- Does **React primitives live** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **react-primitives-live**.
...
Implemented in `generator/pages/react-primitives-live.py`; registered in showcase build. Cross-check markers with inventory output.
