...
Showcase museum page **data-charts-api** (`generator/pages/data-charts-api.py`) documenting Data charts API patterns for KS maintainers and consumers.

## Expected look

- **Charts API** demos show interactive refresh states—loading/error overlays bounded to chart region.
- Tooltips and focus rings remain keyboard reachable ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Dca** markers: emitted roots include both `hash="Dca"` and `data-ks-hash="Dca"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `data_charts_api.py` composes inside the advertised parent layout; built HTML for slug `data-charts-api` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Data charts API** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **data-charts-api**.
...
Implemented in `generator/pages/data-charts-api.py`; registered in showcase build. Cross-check markers with inventory output.
