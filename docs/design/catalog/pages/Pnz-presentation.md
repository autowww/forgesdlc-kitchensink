...
status: "active"
source_paths:
  - generator/pages/presentation.py
showcase_url: "https://ks.forgesdlc.com/cases/showcase/presentation.html"
screenshot_url: "https://ks.forgesdlc.com/cases/showcase/screenshots/Pnz.png"
...
- **Type:** page
- **Category:** page
- **Source paths:** `generator/pages/presentation.py`
- **Showcase URL / status:** https://ks.forgesdlc.com/cases/showcase/presentation.html (active preview page)
- **Screenshot URL / status:** https://ks.forgesdlc.com/cases/showcase/screenshots/Pnz.png — status **planned**

## Purpose

Showcase museum page **presentation** (`generator/pages/presentation.py`) documenting Presentation patterns for KS maintainers and consumers.

## Expected look

- **Presentation** patterns show slide-scale typography and hero bands suited to speaker-view framing.
- Motion (if present) stays bounded and pairs with reduced-motion fallbacks ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Pnz** markers: emitted roots include both `hash="Pnz"` and `data-ks-hash="Pnz"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `presentation.py` composes inside the advertised parent layout; built HTML for slug `presentation` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Presentation** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **presentation**.
...
Implemented in `generator/pages/presentation.py`; registered in showcase build. Cross-check markers with inventory output.
