---
hash: "Dcs"
name: "Data charts static"
type: "page"
status: "active"
source_paths:
  - generator/pages/data_charts_static.py
showcase_url: "https://ks.forgesdlc.com/cases/showcase/data-charts-static.html"
screenshot_url: "https://ks.forgesdlc.com/cases/showcase/screenshots/Dcs.png"
screenshot_status: "planned"
---

# Dcs — Data charts static
...
- **Type:** page
- **Category:** page
- **Source paths:** `generator/pages/data_charts_static.py`
- **Showcase URL / status:** https://ks.forgesdlc.com/cases/showcase/data-charts-static.html (active preview page)
- **Screenshot URL / status:** https://ks.forgesdlc.com/cases/showcase/screenshots/Dcs.png — status **planned**

## Purpose

Showcase museum page **data-charts-static** (`generator/pages/data-charts-static.py`) documenting Data charts static patterns for KS maintainers and consumers.

## Expected look

- **Static charts** demonstrate **Dcs** styling—axes, ticks, and legends readable without animation.
- Color palettes respect semantic consistency across chart types ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Dcs** markers: emitted roots include both `hash="Dcs"` and `data-ks-hash="Dcs"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `data_charts_static.py` composes inside the advertised parent layout; built HTML for slug `data-charts-static` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Data charts static** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **data-charts-static**.
...
Implemented in `generator/pages/data-charts-static.py`; registered in showcase build. Cross-check markers with inventory output.
