...
Showcase museum page **navigation** (`generator/pages/navigation.py`) documenting Navigation patterns patterns for KS maintainers and consumers.

## Expected look

- **Navigation patterns** page compares IA treatments—tabs, pills, breadcrumbs coexist without duplicate mastheads.
- Highlights differences between global (**Kpn**), doc (**Ksr**), and in-page navigation ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Nav** markers: emitted roots include both `hash="Nav"` and `data-ks-hash="Nav"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `navigation.py` composes inside the advertised parent layout; built HTML for slug `navigation` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Navigation patterns** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **navigation**.
...
Implemented in `generator/pages/navigation.py`; registered in showcase build. Cross-check markers with inventory output.
