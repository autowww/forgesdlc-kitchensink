...
Showcase museum page **split-layout** (`generator/pages/split-layout.py`) documenting Split layout demo patterns for KS maintainers and consumers.

## Expected look

- **Split layout demo** stresses two-pane ergonomics—divider visibility and pane hierarchy clear.
- Collapsed mobile behavior should mirror **Spl** expectations ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Slt** markers: emitted roots include both `hash="Slt"` and `data-ks-hash="Slt"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `split_layout_demo.py` composes inside the advertised parent layout; built HTML for slug `split-layout` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Split layout demo** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **split-layout**.
...
Implemented in `generator/pages/split-layout.py`; registered in showcase build. Cross-check markers with inventory output.
