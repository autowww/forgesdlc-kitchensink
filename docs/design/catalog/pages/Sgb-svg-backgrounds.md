...
status: "active"
source_paths:
  - generator/pages/svg_backgrounds.py
showcase_url: "https://ks.forgesdlc.com/cases/showcase/svg-backgrounds.html"
screenshot_url: "https://ks.forgesdlc.com/cases/showcase/screenshots/Sgb.png"
...
- **Type:** page
- **Category:** page
- **Source paths:** `generator/pages/svg_backgrounds.py`
- **Showcase URL / status:** https://ks.forgesdlc.com/cases/showcase/svg-backgrounds.html (active preview page)
- **Screenshot URL / status:** https://ks.forgesdlc.com/cases/showcase/screenshots/Sgb.png — status **planned**

## Purpose

Showcase museum page **svg-backgrounds** (`generator/pages/svg-backgrounds.py`) documenting SVG backgrounds patterns for KS maintainers and consumers.

## Expected look

- **SVG backgrounds** gallery highlights atmospheric layers behind content—foreground text stays dominant.
- Motion-capable backgrounds defer to reduced-motion settings ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Sgb** markers: emitted roots include both `hash="Sgb"` and `data-ks-hash="Sgb"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `svg_backgrounds.py` composes inside the advertised parent layout; built HTML for slug `svg-backgrounds` includes the nested layout hash markers expected by showcase inventory.
...
- Does **SVG backgrounds** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **svg-backgrounds**.
...
Implemented in `generator/pages/svg-backgrounds.py`; registered in showcase build. Cross-check markers with inventory output.
