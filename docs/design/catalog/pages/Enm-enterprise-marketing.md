...
showcase_url: "https://ks.forgesdlc.com/cases/showcase/enterprise-marketing.html"
screenshot_url: "https://ks.forgesdlc.com/cases/showcase/screenshots/Enm.png"
...
- **Showcase URL / status:** https://ks.forgesdlc.com/cases/showcase/enterprise-marketing.html (active preview page)
- **Screenshot URL / status:** https://ks.forgesdlc.com/cases/showcase/screenshots/Enm.png — status **planned**
...
- Avoids gimmick gradients; accent usage matches disciplined amber/cyan ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Enm** markers: emitted roots include both `hash="Enm"` and `data-ks-hash="Enm"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `enterprise_marketing.py` composes inside the advertised parent layout; built HTML for slug `enterprise-marketing` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Enterprise marketing** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **enterprise-marketing**.
