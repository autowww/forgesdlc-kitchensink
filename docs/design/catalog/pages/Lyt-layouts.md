...
Showcase museum page **layouts** (`generator/pages/layouts.py`) documenting Layouts demo patterns for KS maintainers and consumers.

## Expected look

- **Layouts demo** indexes layout families with readable thumbnails/cards—differences between handbook/landing/gallery obvious.
- Cross-links to preview hashes for screenshot parity ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Lyt** markers: emitted roots include both `hash="Lyt"` and `data-ks-hash="Lyt"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `layouts_demo.py` composes inside the advertised parent layout; built HTML for slug `layouts` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Layouts demo** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **layouts**.
...
Implemented in `generator/pages/layouts.py`; registered in showcase build. Cross-check markers with inventory output.
