...
Showcase museum page **index** (`generator/pages/index.py`) documenting Showcase home patterns for KS maintainers and consumers.

## Expected look

- **Showcase home / index**: curated portal into museum sections—cards or lists introduce areas without dumping raw link walls.
- First screen sells scan structure; dense references stay below the fold ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Idx** markers: emitted roots include both `hash="Idx"` and `data-ks-hash="Idx"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `index.py` composes inside the advertised parent layout; built HTML for slug `index` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Showcase home** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **index**.
...
Implemented in `generator/pages/index.py`; registered in showcase build. Cross-check markers with inventory output.
