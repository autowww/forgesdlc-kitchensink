...
Showcase museum page **controls** (`generator/pages/controls.py`) documenting Controls patterns for KS maintainers and consumers.
...
- Components align to KS forms/button guidance without one-off styling ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Ctr** markers: emitted roots include both `hash="Ctr"` and `data-ks-hash="Ctr"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `controls.py` composes inside the advertised parent layout; built HTML for slug `controls` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Controls** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **controls**.
...
Implemented in `generator/pages/controls.py`; registered in showcase build. Cross-check markers with inventory output.
