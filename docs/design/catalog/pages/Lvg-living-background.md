...
showcase_url: "https://ks.forgesdlc.com/cases/showcase/living-background.html"
screenshot_url: "https://ks.forgesdlc.com/cases/showcase/screenshots/Lvg.png"
...
- **Showcase URL / status:** https://ks.forgesdlc.com/cases/showcase/living-background.html (active preview page)
- **Screenshot URL / status:** https://ks.forgesdlc.com/cases/showcase/screenshots/Lvg.png — status **planned**
...
- Respects reduced-motion and prefers CSS transforms over layout thrash ([forge-enterprise-ai-website-standard.md](../../../design/forge-enterprise-ai-website-standard.md)).
...
- Showcase/build output honors **Lvg** markers: emitted roots include both `hash="Lvg"` and `data-ks-hash="Lvg"` where `emit_marker_in_showcase` / museum rules apply.
...
- Page generator `living_background_system.py` composes inside the advertised parent layout; built HTML for slug `living-background` includes the nested layout hash markers expected by showcase inventory.
...
- Does **Living background system** storytelling match the KS museum intent (education, reassurance, parity with consumer sites), not accidental placeholder copy?
- Are technical blocks (API tables, prose blocks) progressively disclosed consistent with Forge landing doctrine?
- Screenshot or DOM review: hero / first-scroll real estate reinforces the visitor job for **living-background**.
