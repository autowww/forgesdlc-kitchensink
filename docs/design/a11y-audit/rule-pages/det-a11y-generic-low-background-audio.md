---
rule_id: DET.A11Y.GENERIC.LOW_BACKGROUND_AUDIO
lane: deterministic
scope: generic
...
- Auditor emits a finding with `ruleId` `DET.A11Y.GENERIC.LOW_BACKGROUND_AUDIO` on the crawled URL.
- Harness: `auditor-tests/invoke-a11y-ruleset-harness.sh DET.A11Y.GENERIC.LOW_BACKGROUND_AUDIO` expects ≥1 finding on the Before fixture.
...
1. Reproduce with `analyze-website-a11y.mjs --only-deterministic-rule-ids DET.A11Y.GENERIC.LOW_BACKGROUND_AUDIO` when lane is deterministic.
