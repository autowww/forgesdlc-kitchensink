---
rule_id: DET.A11Y.GENERIC.USE_OF_COLOR
lane: deterministic
scope: generic
title: Use Of Color
...
- Auditor emits a finding with `ruleId` `DET.A11Y.GENERIC.USE_OF_COLOR` on the crawled URL.
- Harness: `auditor-tests/invoke-a11y-ruleset-harness.sh DET.A11Y.GENERIC.USE_OF_COLOR` expects ≥1 finding on the Before fixture.
...
1. Reproduce with `analyze-website-a11y.mjs --only-deterministic-rule-ids DET.A11Y.GENERIC.USE_OF_COLOR` when lane is deterministic.
