---
rule_id: DET.A11Y.GENERIC.LANG_OF_PARTS
lane: deterministic
scope: generic
title: Lang Of Parts
...
- Auditor emits a finding with `ruleId` `DET.A11Y.GENERIC.LANG_OF_PARTS` on the crawled URL.
- Harness: `auditor-tests/invoke-a11y-ruleset-harness.sh DET.A11Y.GENERIC.LANG_OF_PARTS` expects ≥1 finding on the Before fixture.
...
1. Reproduce with `analyze-website-a11y.mjs --only-deterministic-rule-ids DET.A11Y.GENERIC.LANG_OF_PARTS` when lane is deterministic.
